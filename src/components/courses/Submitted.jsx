...
const Submitted = ({
  ...
  useEffect(() => {
    if (course) {
      const basePath = `https://flea-helped-locust.ngrok-free.app/course`;
      ...
      try {
        const promises = submittedItems.flatMap(item =>
          item.solutions?.map(solution => axios.get(
            `https://flea-helped-locust.ngrok-free.app/course/${itemType}/solution/${currentCourseID}/${item._id}/${solution._id}`,
