...
const Admin = ({ logout, image, error, isLoading, token, profile }) => {
  ...
  const onSubmitUser = async (e) => {
    ...
    try {
      ...
      dataToSend.append('courseIds', JSON.stringify(courseIds));
      if (files > 0) {
        files.forEach((file, index) => {
          dataToSend.append(`registerImage[${index}]`, file);
        });
      }
      console.log(dataToSend)
      console.log(name, courseIds,role,password,email,files)
  ...
  const onChangeCourse = (e) => {
    ...
    setFormDataCourse({
      ...formDataCourse,
      [name]: value,
      coursePath: generateCoursePath(value)
    });
  };
  ...
  const onSubmitCourse = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('https://thankful-ample-shrimp.ngrok-free.app/course', {
        courseId: formDataCourse.courseId,
        courseName: formDataCourse.courseName,
        coursePath: formDataCourse.coursePath,
        teacherId: formDataCourse.teacherId
      });

      setNotification({ show: true, message: 'Course created successfully!', variant: 'success' });
      setFormDataCourse({ courseId: '', courseName: '', coursePath: '' });
    } catch (error) {
                ...
                <Form onSubmit={onSubmitCourse}>
                  ...
                  <Form.Group controlId="formCourseTeacherId">
                    ...
                    <Form.Control
                      type="text"
                      name="teacherId"
                      value={formDataCourse.teacherId}
                      onChange={onChangeCourse}
                      required
                    />
                  </Form.Group>
